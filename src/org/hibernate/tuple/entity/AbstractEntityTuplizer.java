...
public abstract class AbstractEntityTuplizer implements EntityTuplizer {
	...
	public AbstractEntityTuplizer(EntityMetamodel entityMetamodel, PersistentClass mappingInfo) {
		...
		DirectPropertyAccessor dAccessor = new DirectPropertyAccessor();
		Class<?> type = GolemFactory.getGolemClass(entityMetamodel.getName());
		
		
		if ( !entityMetamodel.getIdentifierProperty().isVirtual() ) {
			idGetter = dAccessor.getGetter(type, mappingInfo.getIdentifierProperty().getName());
			idSetter = dAccessor.getSetter(type, mappingInfo.getIdentifierProperty().getName());
		}
		else {
			idGetter = null;
			idSetter = null;
		...
		while ( itr.hasNext() ) {
			...
			getters[i] = dAccessor.getGetter(type, property.getName());
			setters[i] = dAccessor.getSetter(type, property.getName());
	...
	public Serializable getIdentifier(Object entity, SessionImplementor session) {
		...
		if ( entityMetamodel.getIdentifierProperty().isEmbedded() ) {
			...
			if ( idGetter == null ) {
				...
				id = new DirectPropertyAccessor().getGetter(entity.getClass(), entityMetamodel.getIdentifierProperty().getName()).get(entity);
	...
	public void setIdentifier(Object entity, Serializable id, SessionImplementor session) {
		if ( entityMetamodel.getIdentifierProperty().isEmbedded() ) {
			...
			new DirectPropertyAccessor().getSetter(entity.getClass(), entityMetamodel.getIdentifierProperty().getName()).set(entity, id, getFactory());
		}
	...
	public Object[] getPropertyValues(Object entity) throws HibernateException {
		...
		for ( int j = 0; j < span; j++ ) {
			...
			if ( getAll || !property.isLazy() ) {
				result[j] = new DirectPropertyAccessor().getGetter(entity.getClass(), entityMetamodel.getPropertyNames()[j] ).get( entity );
			}
			else {
	...
	throws HibernateException {
		...
		for ( int j = 0; j < span; j++ ) {
			result[j] = new DirectPropertyAccessor().getGetter(entity.getClass(), entityMetamodel.getPropertyNames()[j] ).getForInsert( entity, mergeMap, session );
		}
		return result;
	}

	public Object getPropertyValue(Object entity, int i) throws HibernateException {
		return new DirectPropertyAccessor().getGetter(entity.getClass(), entityMetamodel.getPropertyNames()[i]).get( entity );
	}
	...
	public void setPropertyValues(Object entity, Object[] values) throws HibernateException {
		...
		for ( int j = 0; j < entityMetamodel.getPropertySpan(); j++ ) {
			if ( setAll || values[j] != LazyPropertyInitializer.UNFETCHED_PROPERTY ) {
				new DirectPropertyAccessor().getSetter(entity.getClass(), entityMetamodel.getPropertyNames()[j] ).set(entity,values[j], getFactory());
	...
	public void setPropertyValue(Object entity, int i, Object value) throws HibernateException {
		new DirectPropertyAccessor().getSetter(entity.getClass(),  entityMetamodel.getPropertyNames()[i]).set(entity,value, getFactory());
